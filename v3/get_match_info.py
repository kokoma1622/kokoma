...
def match_info(match, summoner_list):
    ...
    win_dict = {'Win':'BLUE', 'Fail':'RED'}        
    win = win_dict[pd.DataFrame(match['teams'][0]).iloc[0]['win']]
    
    match_dict = {'gameID':match['gameId'].item(), 'season':season, 'version':version, 'winTeam':win, 'averageTier':tier, 
                  'startTime':match['gameCreation'], 'duration':match['gameDuration']}
