...
def build_model(input_size):
  ...
  model.add(layers.Dense(200, activation='sigmoid', input_shape=(input_size,)))
...
def get_data():
  ...
  table_column_list = {'result': [0, 1, 2, 3], 'detail': [0, 3, 10], 'laning': [1, 2, 3, 4, 5, 6]}
  ...
  for db in db_list:
    ...
    for table in table_list:
      data_self_zscore[db][table] = np.load('dataset/s11/v01/%s/zscore_self_%s.npy' % (db, table))[:, table_column_list[table]]
      data_diff_zscore[db][table] = np.load('dataset/s11/v01/%s/zscore_diff_%s.npy' % (db, table))[:, table_column_list[table]]
